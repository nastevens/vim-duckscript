...
syntax match duckKeyword display contained "\v<%(read_properties|release|remove_command|replace|rm|rmdir)>"
syntax match duckKeyword display contained "\v<%(scope_pop_stack|scope_push_stack|set|set_by_name|set_clear)>"
syntax match duckKeyword display contained "\v<%(set_contains|set_env|set_error|set_from_array|set_is_empty)>"
syntax match duckKeyword display contained "\v<%(set_new|set_put|set_add|set_remove|set_size|set_to_array)>"
syntax match duckKeyword display contained "\v<%(sleep|spawn|split|starts_with|string_to_bytes|substring)>"
syntax match duckKeyword display contained "\v<%(temp_dir|temp_file|test_directory|touch|trigger_error|trim)>"
syntax match duckKeyword display contained "\v<%(trim_end|trim_start|unalias|uname|unset|unset_env|watchdog)>"
syntax match duckKeyword display contained "\v<%(wget|which|whoami|get_user_name|writebinfile)>"
