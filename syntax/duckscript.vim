...
syntax match duckKeyword display contained "\v<%(get_last_error_source|get_last_modified_time)>"
syntax match duckKeyword display contained "\v<%(gitignore_path_array|glob_array|globarray|glob_chmod)>"
syntax match duckKeyword display contained "\v<%(chmod_glob|glob_cp|cp_glob|goto|greater_than|hex_decode)>"
syntax match duckKeyword display contained "\v<%(hex_encode|hostname|http_client|if|indexof|is_array)>"
syntax match duckKeyword display contained "\v<%(is_command_defined|is_defined|is_directory|is_dir|is_empty)>"
syntax match duckKeyword display contained "\v<%(is_file|is_map|is_path_exists|is_path_newer|is_readonly)>"
syntax match duckKeyword display contained "\v<%(is_set|is_windows|join_path|json_encode|json_parse)>"
syntax match duckKeyword display contained "\v<%(kebabcase|last_indexof|length|strlen|less_than|lowercase|ls)>"
...
syntax match duckKeyword display contained "\v<%(print|print_env|printenv|println|pwd)>"
syntax match duckKeyword display contained "\v<%(print_current_directory|random_range|rand_range|random_text)>"
syntax match duckKeyword display contained "\v<%(rand_text|range|read|readbinfile|read_binary_file|readfile)>"
syntax match duckKeyword display contained "\v<%(read_text_file|read_properties|release|remove_command)>"
syntax match duckKeyword display contained "\v<%(replace|rm|rmdir|scope_pop_stack|scope_push_stack)>"
syntax match duckKeyword display contained "\v<%(semver_is_equal|semver_is_newer|semver_parse|set)>"
...
syntax match duckKeyword display contained "\v<%(set_remove|set_size|set_to_array|sha256sum|sha256sum)>"
syntax match duckKeyword display contained "\v<%(sha512sum|sha512sum|sleep|snakecase|spawn|split|starts_with)>"
syntax match duckKeyword display contained "\v<%(string_to_bytes|substring|temp_dir|temp_file|test_directory)>"
syntax match duckKeyword display contained "\v<%(test_file|touch|trigger_error|trim|trim_end|trim_start)>"
syntax match duckKeyword display contained "\v<%(unalias|uname|unset|unset_all_vars|unset_env|uppercase)>"
