...
syntax match duckKeyword display contained "\v<%(exec|exit|quit|q|exit_on_error|set_exit_on_error|for)>"
syntax match duckKeyword display contained "\v<%(function|fn|get_all_var_names|get_by_name|get_env)>"
syntax match duckKeyword display contained "\v<%(get_home_dir|get_last_error|get_last_error_line)>"
syntax match duckKeyword display contained "\v<%(get_last_error_source|glob_array|globarray|glob_chmod|goto)>"
syntax match duckKeyword display contained "\v<%(greater_than|hostname|http_client|if|indexof|is_array)>"
syntax match duckKeyword display contained "\v<%(is_defined|is_directory|is_dir|is_empty|is_file|is_map)>"
syntax match duckKeyword display contained "\v<%(is_path_exists|is_readonly|is_set|is_windows|last_indexof)>"
syntax match duckKeyword display contained "\v<%(length|strlen|less_than|ls|man|map|map_clear)>"
...
syntax match duckKeyword display contained "\v<%(map_size|map_to_properties|mkdir|mv|not|os_family|os_name)>"
syntax match duckKeyword display contained "\v<%(os_release|os_version|pid|process_id|print_env|printenv|pwd)>"
syntax match duckKeyword display contained "\v<%(print_current_directory|range|read|read_properties)>"
syntax match duckKeyword display contained "\v<%(readbinfile|read_binary_file|readfile|read_text_file)>"
syntax match duckKeyword display contained "\v<%(release|remove_command|replace|rm|rmdir|set|set_by_name)>"
syntax match duckKeyword display contained "\v<%(set_clear|set_contains|set_env|set_error|set_from_array)>"
syntax match duckKeyword display contained "\v<%(set_is_empty|set_new|set_put|set_add|set_remove|set_size)>"
syntax match duckKeyword display contained "\v<%(set_to_array|sleep|spawn|split|starts_with|string_to_bytes)>"
syntax match duckKeyword display contained "\v<%(substring|temp_dir|temp_file|test_directory|touch)>"
syntax match duckKeyword display contained "\v<%(trigger_error|trim|trim_end|trim_start|unalias|uname|unset)>"
syntax match duckKeyword display contained "\v<%(unset_env|watchdog|wget|which|whoami|get_user_name)>"
syntax match duckKeyword display contained "\v<%(write_properties|writebinfile|write_binary_file|writefile)>"
syntax match duckKeyword display contained "\v<%(write_text_file)>"
